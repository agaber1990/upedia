    ...
    public function up(): void
    {
        ...
        foreach ($filesInFolder as $path) {
            ...
            if (file_exists($file['dirname'] . '/' . $file['basename'])) {
                ...
                if ($file_data) {
                    $check_exist  = DB::table(config('pagebuilder.db_prefix', 'upedia__') . 'pages')->where('school_id', 1)->where('slug', $file_data['slug'])->first();
                    if (!$check_exist) {
                        DB::table(config('pagebuilder.db_prefix', 'upedia__') . 'pages')->insert(
                            [
                                'name' => $file_data['name'],
        ...
        $frontResultDatas = [
            'Science' => '/uploads/front_result/sci.jpg', 
            'Arts' => '/uploads/front_result/art.jpg', 
            'Commerce' => '/uploads/front_result/comm.png'
        ];
