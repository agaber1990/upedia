...
<head>
    ...
    @endif
    <title>{{@schoolConfig()->school_name ? @schoolConfig()->school_name : 'upedia ERP'}} |
        @yield('title')
    </title>
...
@php
    if(session()->has('homework_zip_file')){
        $file_path='/uploads/homeworkcontent/'.session()->get('homework_zip_file');
...
<body class="admin"  style="{!! $css !!}">
    ...
    @php
        if (file_exists($generalSetting->logo)) {
            $tt = base_path($generalSetting->logo);
        } else {
            $tt = base_path('/uploads/settings/logo.png');
        }
    @endphp
