    ...
    public function staffPicStore(Request $r)
    {
        ...
        try {
            ...
            if ($r->hasFile('logo_pic')) {
                ...
                $pathImage = '/uploads/staff/';
                if (!file_exists($pathImage)) {
                    mkdir($pathImage, 0777, true);
                    $name = md5($file->getClientOriginalName() . time()) . "." . "png";
                    $images->save('/uploads/staff/' . $name);
                    $imageName = '/uploads/staff/' . $name;
                ...
                if ($request->customF) {
                    ...
                    foreach ($dataImage as $label => $field) {
                        if (is_object($field) && $field != "") {
                            $dataImage[$label] = fileUpload($field, '/uploads/customFields/');
                        }
                    }
        ...
        try {
            if ($request->file('staff_upload_document') != "" && $request->title != "") {
                ...
                if ($request->file('staff_upload_document') != "") {
                    ...
                    $document_photo = '/uploads/staff/document/' . $document_photo;
                }
        ...
        try {
            if ($request->title != "") {
                ...
                if ($request->file('document_file_4') != "") {
                    ...
                    $file->move('/uploads/staff/timeline/', $document_photo);
                    $document_photo = '/uploads/staff/timeline/' . $document_photo;
                }
